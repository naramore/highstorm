defmodule Delve.Planner do
  @moduledoc """
  Core logic for resolver path accumulation.

  Responsible for interfacing with `Delve.Graph` via its `ast_walker/4` and
  `attr_walker/4` callbacks and constructing the paths needed by
  `Delve.Plan` to create the run graph to be used by `Delve.Processor`.
  ...
  @type t :: %__MODULE__{
          available_data: Delve.shape_descriptor(),
          resolver_trail: [{Plan.landmark(), Graph.node_id(), Delve.attr()}],
          paths: %{optional(Delve.attr()) => [{Plan.landmark(), Graph.node_id(), Delve.attr()}]},
          plan: Digraph.t(),
          current_attr: Delve.attr() | nil,
          params: map | nil
        }
  ...
  @spec add_trail(t, Plan.landmark(), Graph.node_id(), Delve.attr()) :: t
  ...
  @spec add_path(t, Plan.landmark(), Graph.node_id(), Delve.attr(), keyword) :: t
  ...
  @spec pre_process_path([{Plan.landmark(), Graph.node_id(), Delve.attr()}]) :: [
          {Plan.landmark(), Graph.node_id(), Delve.attr(), Plan.scope()}
        ]
  ...
  @spec backtrack_and_mark(t, Delve.attr(), [Vertex.t()]) :: {:ok, t} | {:error, reason :: term}
  ...
  @spec backtrack_and_marker(Vertex.t(), acc, Delve.attr()) :: acc
  ...
  @spec update_output(t, Vertex.id(), Delve.attr()) :: t
  ...
  @spec backtrack(t, Delve.attr(), [Vertex.t()]) :: {:ok, t} | {:error, reason :: term}
  defp backtrack(planner, attr, vertices) do
    Enum.reduce(vertices, {:ok, planner}, &backtracker(&1, &2, attr))
  end

  @spec backtracker(Vertex.t(), acc, Delve.attr()) :: acc
        when acc: {:ok, t} | {:error, reason :: term}
  ...
  @spec update_attrs(t, Vertex.id(), Delve.attr()) :: t
