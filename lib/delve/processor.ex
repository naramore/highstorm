defmodule Delve.Processor do
  @moduledoc false

  @type entity :: Delve.response_form()

  @callback process_ast(Delve.env(), EQL.AST.Query.t()) ::
              {:ok, entity} | {:error, reason :: term}

  @spec process_expr(module, Delve.env(), EQL.query()) :: {:ok, entity} | {:error, reason :: term}
  ...
  defmacro __using__(_opts) do
    quote do
      alias Delve.Processor
      @behaviour Processor

      @spec process_expr(Delve.env(), EQL.query()) ::
...
defmodule Delve.Processor.Serial do
  @moduledoc false
  ...
  #   planner: Delve.Planner.t
  #   entity: Delve.Processor.entity
  #   context: map

  # graph = Delve.Graph.new(resolvers)
  # planner = Delve.Planner.new(available_data)
  # {:ok, {graph, planner}} = Delve.Planner.walk_ast(graph, query_ast, planner)
