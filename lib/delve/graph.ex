defmodule Delve.Graph do
  ...
  *Reverse* because this traverses in the opposite direction
  ...
  @type node_id :: Delve.attr() | [Delve.attr()]
  ...
  @type label_tuple(id) :: {id, depth, [Delve.attr()], Delve.attr() | nil, leaf? :: boolean}
  @type label(id) :: %{
          required(:id) => id,
          required(:depth) => depth,
          required(:path) => [Delve.attr()],
          required(:union_key) => Delve.attr() | nil,
  ...
  @spec mark_unreachable(t, t, Delve.attr()) :: t
  ...
  @spec output_info([EQL.AST.t()] | EQL.AST.t(), depth, path, union_key) ::
        ...
        when id: Delve.attr(), path: [id], union_key: id | nil, leaf?: boolean
  ...
  defmacro __using__(_opts) do
    quote do
      @behaviour Delve.Graph

      @spec walk_ast(Delve.Graph.t(), EQL.AST.t(), Delve.Graph.acc()) ::
              {:ok, {Delve.Graph.t(), Delve.Graph.acc()}} | {:error, reason :: term}
      def walk_ast(graph, ast, acc) do
        Delve.Graph.walk_ast(graph, ast, acc, __MODULE__)
      end

      @spec walk_attr(Delve.Graph.t(), Delve.Graph.node_id(), Delve.Graph.acc()) ::
              {Delve.Graph.reachability(), Delve.Graph.t(), Delve.Graph.acc()}
      def walk_attr(graph, node_id, acc) do
        Delve.Graph.walk_attr(graph, node_id, acc, __MODULE__)
      end
    end
  end
end
