defmodule Entry.Improper do
  ...
  ## Examples
    ...
    iex> Entry.Improper.improper_get([:a, :b | :c], 1)
    :b

    # improper tail is 1 greater (i.e. 2)
    iex> Entry.Improper.improper_get([:a, :b | :c], 2)
    :c
    ```
  """
...
defmodule Entry do
  @moduledoc """
  `Entry` is shorthand for `Access` and overrides some of the default behaviour
  ...
  * an improper list's tail may be specified as the last element in the 'list'
  ...
  By using Entry, you may specify which of these functions to override using
  ...
  defmodule B do
    use Entry, only: get_in: 2
  end

  defmodule C do
    import Kernel, except: [get_in: 2]
    import Entry, only: [get_in: 2]
  end
  ```
  """

  use Boundary, deps: [], exports: [Improper]
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      import Entry, only: unquote(opts)
    end
  end
  ...
  Structs, lists, and tuples require less boilerplate to access.
    ...
    ...> Entry.get_and_update_in(data, lazy_keys, fun) == Kernel.get_and_update_in(data, keys, fun)
    true
    ```
  ...
  an exception.
    ...
    ```
    iex> Entry.get_and_update_in(%{}, [], fn x -> {x, Map.put(x, :a, 1)} end)
    {%{}, %{a: 1}}
    ```
  ...
  `length(data)` of the list.
    ...
    ```
    iex> Entry.get_and_update_in([:a, :b, :c | :d], [3], fn x -> {x, :e} end)
    {:d, [:a, :b, :c | :e]}
    ```
  """
  ...
  Structs, lists, and tuples require less boilerplate to access.
    ...
    ...> lazy_keys = [0, 1, :message]
    ...> keys = [Access.at(0), Access.elem(1), Access.key(:message)]
    ...> Entry.get_in(data, lazy_keys) == Kernel.get_in(data, keys)
    true
    ```
  ...
  Structs, lists, and tuples require less boilerplate to access.
    ...
    ...> keys = [Access.at(0), Access.elem(1), Access.key(:message)]
    ...> fun = fn x -> x <> "bar" end
    ...> Entry.update_in(data, lazy_keys, fun) == Kernel.update_in(data, keys, fun)
    true
    ```
  ...
  an exception.
    ...
    ```
    iex> Entry.update_in(%{}, [], &Map.put(&1, :a, 1))
    %{a: 1}
    ```
  ...
  `length(data)` of the list.
    ...
    ```
    iex> Entry.update_in([:a, :b, :c | :d], [3], fn _ -> :e end)
    [:a, :b, :c | :e]
    ```
  """
  ...
  Structs, lists, and tuples require less boilerplate to access.
    ...
    ...> lazy_keys = [0, 1, :message]
    ...> keys = [Access.at(0), Access.elem(1), Access.key(:message)]
    ...> Entry.put_in(data, lazy_keys, "bar") == Kernel.put_in(data, keys, "bar")
    true
    ```
  ...
  Structs, lists, and tuples require less boilerplate to access.
    ...
    ...> lazy_keys = [0, 1, :message]
    ...> keys = [Access.at(0), Access.elem(1), Access.key(:message)]
    ...> Entry.pop_in(data, lazy_keys) == Kernel.pop_in(data, keys)
    true
    ```
